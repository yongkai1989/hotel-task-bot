...
export default function CommissionCheckerPage({ standalone = false }: { standalone?: boolean }) {
  ...
  async function handlePublicSignOut() {
    await fetch('/api/commission-checker/auth/logout', { method: 'POST' });
    window.location.assign('/commission-checker/login');
  }
  ...
  return (
    <main className="cc-shell">
      <section className="cc-hero">
        ...
        {standalone ? (
          <button type="button" className="cc-secondary-link" onClick={() => void handlePublicSignOut()}>
            Sign out
          </button>
        ) : (
