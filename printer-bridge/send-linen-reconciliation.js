...
async function main() {
  ...
  const response = await fetch(
    `${appOrigin(config.appUrl)}/api/operational-reminders?kind=linen-reconciliation`,
    {
      method: 'GET',
      headers: {
  ...
  if (!response.ok || payload?.ok === false) {
    throw new Error(payload?.error || `Linen reconciliation request failed: ${response.status}`);
  }

  console.log(
    payload.alreadySent
      ? 'Today\'s 1:00 PM Linen Reconciliation was already processed.'
      : payload.status === 'NOT_NEEDED'
        ? 'Linen Reconciliation was not sent because Laundry Received was not 2/2 at 1:00 PM.'
        : `Linen Reconciliation sent in ${Number(payload.attempted || 1)} Telegram message(s).`
  );
}
