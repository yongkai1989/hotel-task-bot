...
async function main() {
  ...
  const response = await fetch(
    `${appOrigin(config.appUrl)}/api/operational-reminders?kind=chambermaid`,
    {
      method: 'GET',
      headers: {
  ...
  if (!response.ok || payload?.ok === false) {
    throw new Error(payload?.error || `Reminder request failed: ${response.status}`);
  }

  if (payload.alreadySent) {
    console.log('The 5:00 PM Chambermaid reminder was already processed today.');
  } else if (payload.findingCount) {
    console.log(`Reminder sent for ${payload.findingCount} unsaved room(s).`);
  } else {
    console.log('All required rooms were saved. No reminder was sent.');
  }
}
