create or replace function public.update_hk_special_project_room_checklist(
  p_task_id uuid,
  p_run_id uuid,
  p_room_numbers text[]
)
...
begin
  if auth.uid() is null or not exists (
    ...
    raise exception 'You do not have permission to edit HK Special Project room checklists';
  end if;
  ...
  if v_run_status = 'DONE' then
    raise exception 'Reopen this task before editing its room checklist';
  end if;
...
revoke all on function public.update_hk_special_project_room_checklist(uuid, uuid, text[]) from public;
revoke all on function public.update_hk_special_project_room_checklist(uuid, uuid, text[]) from anon;
grant execute on function public.update_hk_special_project_room_checklist(uuid, uuid, text[]) to authenticated;
