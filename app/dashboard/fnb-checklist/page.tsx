...
function getFNBChecklistServiceDateString() {
  const d = new Date();
  if (d.getHours() < 12) {
...
export default function FNBChecklistPage() {
  const supabase = useMemo(() => getSupabaseSafe(), []);
  const today = getFNBChecklistServiceDateString();
    ...
    async function bootstrap() {
      try {
        ...
        const { data: profileRow, error: profileError } = await supabase
          .from('user_profiles')
          .select('user_id, email, name, role, can_access_fnb_checklist')
        ...
        setProfile({
          ...
          can_access_fnb_checklist: profileRow?.can_access_fnb_checklist === true,
        });
      } catch (err: any) {
        if (!mounted) return;
  ...
  async function loadTemplatesAndQuestions() {
    ...
    try {
      ...
      setErrorMsg(err?.message || 'Failed to load F&B Check Lists');
    } finally {
      setLoading(false);
    }
  }
    ...
    void openHistorySubmission(
      {
        ...
        submitted_by_name: row.name || row.email || 'F&B',
              ...
              return (
                ...
                >
                  <div style={styles.trackerUser}>
                    ...
                    <div>
                      <div style={styles.trackerName}>{row.name || row.email || 'F&B'}</div>
    ...
    return (
      <main style={styles.page}>
        <div style={styles.centerCard}>
          ...
          <p style={styles.centerText}>Only authorised F&B checklist users and superusers can access F&B Check List.</p>
          <Link href="/dashboard" style={styles.linkBtn}>Back to Dashboard</Link>
        </div>
      </main>
    );
      ...
      <div style={styles.shell}>
        ...
        {loading ? (
          <section style={{ ...styles.panel, ...(isMobile ? styles.panelMobile : {}) }}>
            <div style={styles.emptyState}>Loading F&B Check Lists...</div>
          </section>
        ) : null}
                ...
                return (
                  ...
                  >
                    ...
                    <div style={styles.ChecklistChooserMeta}>
                      {templateQuestions.length} question{templateQuestions.length === 1 ? '' : 's'} - F&B Check List
                    </div>
                ...
                >
                  ...
                  {question.existingId ? (
                    <div style={styles.lockNotice}>
                      Removing this question will also remove its saved answers from past F&B Check List submissions.
                    </div>
                  ) : null}
                </div>
              ))}
