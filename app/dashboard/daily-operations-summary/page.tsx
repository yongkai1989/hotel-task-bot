  ...
  return (
    <main className="ops-page">
      <header className="ops-header">
        ...
        <div className="header-controls"><label>Report date<input type="date" value={reportDate} onChange={(event) => { setReportDate(event.target.value); setThreeMonthRanking(null); setThreeMonthError(''); }} /></label><button onClick={() => { void loadSummary(); if (rankingPeriod === 'THREE_MONTHS') void loadThreeMonthRanking(); }} disabled={loading || threeMonthLoading}>{loading || threeMonthLoading ? 'Refreshing…' : 'Refresh'}</button><Link href="/dashboard">Dashboard</Link></div>
      </header>
      ...
      <section className="panel">
        ...
        <div className="reconciliation-grid">
          ...
          <section className="reconciliation-card return-card">
            <div className="reconciliation-heading return-heading"><div><strong>Returned linen − matching collection bill</strong><span>CC {summary?.linen?.return_cc_no || 'Not recorded'}</span><div className="comparison-dates"><span className="comparison-date bill-date"><small>Collection Date</small><b>{formatDate(summary?.linen?.return_collection_date || summary?.linen?.previous_bill_service_date)}</b></span><span className="comparison-date service-date"><small>Linen Service Date</small><b>{formatDate(summary?.linen?.return_source_service_date)}</b></span><span className="comparison-date return-date"><small>Return Date</small><b>{formatDate(summary?.report_date)}</b></span></div></div><small>Positive means the return is higher; negative means the supplier bill is higher.</small></div>
            {summary?.linen?.return_saved ? <div className="table-wrap"><table className="linen-table"><thead><tr><th>Linen</th><th>In Bill</th><th>Returned</th><th>Difference</th></tr></thead><tbody>{(summary?.linen?.items || []).map((item) => <tr key={item.key}><td><b>{item.label}</b></td><td>{item.previous_in_bill}</td><td>{item.returned}</td><td><Diff value={item.returned - item.previous_in_bill} /></td></tr>)}</tbody></table></div> : <div className="return-outstanding"><strong>Return still outstanding</strong><span>{summary?.linen?.return_outstanding_cc_no ? `Waiting for ${summary.linen.return_outstanding_cc_no}.` : 'The matching Block 1 and Block 2 CC numbers have not both been marked received.'}</span></div>}
          </section>
        </div>
      </section>
