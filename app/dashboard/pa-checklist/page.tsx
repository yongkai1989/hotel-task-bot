...
function getPAChecklistServiceDateString() {
  const d = new Date();
  if (d.getHours() < 12) {
...
export default function PAChecklistPage() {
  const supabase = useMemo(() => getSupabaseSafe(), []);
  const today = getPAChecklistServiceDateString();
  ...
  async function loadTemplatesAndQuestions() {
    ...
    try {
      ...
      setErrorMsg(err?.message || 'Failed to load PA Checklists');
    } finally {
      setLoading(false);
    }
  }
    ...
    void openHistorySubmission(
      {
        ...
        submitted_by_name: row.name || row.email || 'PA',
              ...
              return (
                ...
                >
                  <div style={styles.trackerUser}>
                    ...
                    <div>
                      <div style={styles.trackerName}>{row.name || row.email || 'PA'}</div>
    ...
    return (
      <main style={styles.page}>
        <div style={styles.centerCard}>
          ...
          <p style={styles.centerText}>Only authorised PA checklist users and superusers can access PA Checklist.</p>
          <Link href="/dashboard" style={styles.linkBtn}>Back to Dashboard</Link>
        </div>
      </main>
    );
      ...
      <div style={styles.shell}>
        <div style={{ ...styles.topBar, ...(isMobile ? styles.topBarMobile : {}) }}>
          <div>
            ...
            <div style={styles.pageTitle}>PA Checklist</div>
            <div style={styles.pageSubTitle}>
              {profile.name} ({profile.role}) - PA checklist workspace
            </div>
          </div>
        ...
        {loading ? (
          <section style={{ ...styles.panel, ...(isMobile ? styles.panelMobile : {}) }}>
            <div style={styles.emptyState}>Loading PA Checklists...</div>
          </section>
        ) : null}
                ...
                return (
                  ...
                  >
                    ...
                    <div style={styles.ChecklistChooserMeta}>
                      {templateQuestions.length} question{templateQuestions.length === 1 ? '' : 's'} - PA Checklist
                    </div>
                ...
                >
                  ...
                  {question.existingId ? (
                    <div style={styles.lockNotice}>
                      Removing this question will also remove its saved answers from past PA Checklist submissions.
                    </div>
                  ) : null}
                </div>
              ))}
