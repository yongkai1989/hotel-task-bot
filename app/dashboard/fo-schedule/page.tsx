...
export default function FrontOfficeSchedulePage(){
  ...
  const restExceptions=useMemo(()=>{
    ...
    for(const entry of ordered){
      ...
      if(previous&&shiftCode(previous)!==shiftCode(entry)){
        ...
        if(hours!==null&&hours<=12)result.set(`${entry.staff_id}:${entry.schedule_date}`,`${hours.toFixed(2)} hours rest · ${shiftCode(previous)} → ${shiftCode(entry)}`);
      }
  ...
  const coverage=useMemo(()=>days.map(day=>{
    ...
    const morningHasBackup=(counts.MORNING||0)===1&&(counts.MANAGER||0)>=1&&(counts.RESERVATIONS||0)>=1;
    const shortages=Object.entries(TARGETS).filter(([code,target])=>!(code==='NIGHT'&&saravananSolo)&&!(code==='MORNING'&&morningHasBackup)&&(counts[code]||0)<target).map(([code,target])=>`${code} ${counts[code]||0}/${target}`);
  ...
  return <main className={styles.page}>
    <section className={styles.hero}><div><span className={styles.eyebrow}>FRONT OFFICE WORKFORCE</span><h1>Front Office Schedule</h1><p>Manual monthly roster with coverage guidance and warnings for shift changes with 12 hours of rest or less.</p></div>
      {canEdit?<div className={styles.heroActions}><span className={styles.viewOnlyBadge}>Manual timetable</span><button className={styles.secondaryButton} onClick={()=>setStaffOpen(true)}>Staff setup</button></div>:<span className={styles.viewOnlyBadge}>View only</span>}
    </section>
...
function StaffModal({staff,busy,onClose,onSave,onDelete}:{staff:Staff[];busy:boolean;onClose:()=>void;onSave:(person:Staff|null,form:StaffForm)=>void;onDelete:(person:Staff)=>void}){
  ...
  return <ModalShell title="Front Office staff & night setup" subtitle="Set each person's role, gender, preferred shift and fixed off day. Select any number of Hybrid Night staff." onClose={onClose}>
    <div className={`${styles.modalBody} ${styles.foStaffBody}`}>
      ...
      <div className={`${styles.infoBox} ${styles.foStaffRuleNote}`}><strong>Manual timetable</strong><span>Changing a staff member's role, preference, gender or fixed off day does not alter any timetable cell. Schedule changes happen only when you save or clear a cell, or use Fill dates yourself.</span></div>
