...
export default function FrontOfficeSchedulePage(){
  ...
  const restExceptions=useMemo(()=>{
    ...
    for(const entry of ordered){
      ...
      if(previous&&shiftCode(previous)!==shiftCode(entry)){
        ...
        if(hours!==null&&hours<16)result.set(`${entry.staff_id}:${entry.schedule_date}`,`${hours.toFixed(2)} hours rest · ${shiftCode(previous)} → ${shiftCode(entry)}`);
      }
  ...
  return <main className={styles.page}>
    <section className={styles.hero}><div><span className={styles.eyebrow}>FRONT OFFICE WORKFORCE</span><h1>Front Office Schedule</h1><p>Night-first monthly roster with coverage targets and 16-hour shift-change rest checks.</p></div>
