...
export default function FrontOfficeSchedulePage(){
  ...
  const coverage=useMemo(()=>days.map(day=>{const counts:Record<string,number>={MORNING:0,AFTERNOON:0,NIGHT:0,RESERVATIONS:0,MID:0};let nightMale=0;for(const person of activeStaff){const e=entryMap.get(`${person.id}:${day.value}`);if(e?.status==='WORK'){const code=shiftCode(e);counts[code]=(counts[code]||0)+1;if(code==='NIGHT'&&person.staff_gender==='MALE')nightMale+=1;}}const shortages=Object.entries(TARGETS).filter(([code,target])=>(counts[code]||0)<target).map(([code,target])=>`${code} ${counts[code]||0}/${target}`);if((counts.NIGHT||0)>0&&nightMale===0)shortages.push('NIGHT male 0/1');return{date:day.value,counts,nightMale,shortages};}),[activeStaff,days,entryMap]);
  ...
  async function autoFill(){ if(!supabase||!canEdit)return; const hybrids=activeStaff.filter(s=>s.is_hybrid_night); const permanent=activeStaff.find(s=>s.is_permanent_night);
    ...
    if(!window.confirm(`Fill only empty dates in ${new Intl.DateTimeFormat('en-MY',{month:'long',year:'numeric'}).format(new Date(`${month}-01T00:00:00`))}?\n\nManual entries will not be changed. Night coverage is filled first, then Reservations, Morning and Afternoon. Extra staff go to Mid.`))return;
