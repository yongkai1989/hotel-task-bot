...
export default function FrontOfficeSchedulePage(){
  ...
  const restExceptions=useMemo(()=>{
    ...
    for(const entry of ordered){
      ...
      if(previous&&shiftCode(previous)!==shiftCode(entry)){
        ...
        if(hours!==null&&hours<15)result.set(`${entry.staff_id}:${entry.schedule_date}`,`${hours.toFixed(2)} hours rest · ${shiftCode(previous)} → ${shiftCode(entry)}`);
      }
      lastWork.set(entry.staff_id,entry);
    }
    return result;
  },[entries]);
  const approvedSoloNightDates=useMemo(()=>{
    const staffById=new Map(activeStaff.map(person=>[person.id,person]));
    const nightByDate=new Map<string,Entry[]>();
    for(const entry of entries){
      if(entry.status!=='WORK'||!['NIGHT','COVER_NOON'].includes(shiftCode(entry)))continue;
      const rows=nightByDate.get(entry.schedule_date)||[];rows.push(entry);nightByDate.set(entry.schedule_date,rows);
    }
    const byWeek=new Map<string,string[]>();
    for(const [date,rows] of nightByDate){
      if(rows.length!==1||staffById.get(rows[0].staff_id)?.staff_name.trim().toLowerCase()!=='saravanan')continue;
      const key=weekKey(date),dates=byWeek.get(key)||[];dates.push(date);byWeek.set(key,dates);
    }
    const approved=new Set<string>();
    for(const dates of byWeek.values())for(const date of dates.sort().slice(0,2))approved.add(date);
    return approved;
  },[activeStaff,entries]);
  const coverage=useMemo(()=>days.map(day=>{
    const counts:Record<string,number>={MORNING:0,AFTERNOON:0,NIGHT:0,RESERVATIONS:0,MID:0,MANAGER:0,COVER_MORNING:0,COVER_NOON:0,COVER_MANAGER:0};
    ...
    for(const person of activeStaff){
      ...
      if(code==='COVER_MANAGER')counts.AFTERNOON+=1;
      if(code==='NIGHT'||code==='COVER_NOON'){
        nightNames.push(person.staff_name.trim().toLowerCase());
        if(person.staff_gender==='MALE')nightMale+=1;
      }
    }
    const saravananSolo=(counts.NIGHT||0)===1&&nightNames.includes('saravanan')&&approvedSoloNightDates.has(day.value);
    const shortages=Object.entries(TARGETS).filter(([code,target])=>!(code==='NIGHT'&&saravananSolo)&&(counts[code]||0)<target).map(([code,target])=>`${code} ${counts[code]||0}/${target}`);
  ...
  }),[activeStaff,approvedSoloNightDates,days,entryMap]);
  ...
  async function autoFill(){ if(!supabase||!canEdit)return; const hybrids=activeStaff.filter(s=>s.is_hybrid_night); const permanent=activeStaff.find(s=>s.is_permanent_night);
    ...
    if(!window.confirm(`Rebuild Auto Fill assignments for ${new Intl.DateTimeFormat('en-MY',{month:'long',year:'numeric'}).format(new Date(`${month}-01T00:00:00`))}?\n\nChanged fixed-off days will be reapplied across the generated month. Unrelated manually saved shifts and leave remain unchanged. Shift changes providing less than 15 hours of rest are avoided unless coverage is impossible. On short-staffed days when Saravanan is working, Auto Fill may use up to two solo Saravanan Night dates per Monday-Sunday week. The released staff member returns to daytime coverage, Cover Morning works 9:00 AM-7:30 PM, Cover Noon works 7:00 PM-3:30 AM, and Walter works Cover Manager 11:00 AM-7:00 PM to protect the afternoon overlap. These staggered cover shifts are never used when Saravanan is absent. Mid remains surplus-only, with Harish first priority. A female staff member is never accepted alone on Night.`))return;
  ...
  return <main className={styles.page}>
    <section className={styles.hero}><div><span className={styles.eyebrow}>FRONT OFFICE WORKFORCE</span><h1>Front Office Schedule</h1><p>Night-first monthly roster with coverage targets and 15-hour shift-change rest checks.</p></div>
    ...
    {tab==='SCHEDULE'?<>
      <section className={styles.legend}><span><i className={styles.workDot}/> Work</span><span><i className={styles.alDot}/> AL</span><span><i className={styles.uplDot}/> UPL</span><span><i className={styles.mcDot}/> MC</span><span><i className={styles.offDot}/> Off</span><span><i className={styles.noShowDot}/> No Show</span><span className={styles.staffCount}>{activeStaff.length} staff</span><small>Yellow = a change of shift with less than 15 hours of rest. Red date = insufficient coverage, no male on Night, or Jia is not paired with Harish/Zaim.</small></section>
      {shortageDays.length?<section className={styles.error}><strong>{shortageDays.length} day{shortageDays.length===1?'':'s'} below required coverage:</strong> {shortageDays.slice(0,8).map(item=>`${displayDate(item.date)} (${item.shortages.join(', ')})`).join(' · ')}{shortageDays.length>8?' …':''}</section>:<div className={styles.success}>All visible dates meet required coverage. Saravanan may cover Night alone on no more than two dates in each Monday-Sunday week; staggered OT cover is used only while he is working.</div>}
...
function StaffModal({staff,busy,onClose,onSave,onDelete}:{staff:Staff[];busy:boolean;onClose:()=>void;onSave:(person:Staff|null,form:StaffForm)=>void;onDelete:(person:Staff)=>void}){
  ...
  return <ModalShell title="Front Office staff & night setup" subtitle="Set each person's role, gender, preferred shift and fixed off day. Select any number of Hybrid Night staff." onClose={onClose}>
    <div className={`${styles.modalBody} ${styles.foStaffBody}`}>
      ...
      <div className={`${styles.infoBox} ${styles.foStaffRuleNote}`}><strong>Night shift safety rule</strong><span>Every active staff member needs a gender before Auto Fill. Each Night shift must include at least one male. Auto Fill recalculates only AUTO cells and never changes a manually saved cell.</span></div>
      ...
      <section className={styles.foStaffList} aria-label="Front Office staff setup">
        {staff.map(person=>{const value:StaffForm={name:person.staff_name,role:person.staff_role,gender:person.staff_gender||'',off:person.fixed_off_day,preference:person.preferred_shift,permanent:person.is_permanent_night,hybrid:person.is_hybrid_night,manager:person.is_manager,active:person.is_active};return <article className={styles.foStaffCard} key={person.id}>
          ...
          <div className={styles.foStaffControlGrid}>
            ...
            <label>Fixed off day<select value={person.fixed_off_day||''} disabled={busy} onChange={e=>submit(person,{...value,off:e.target.value?Number(e.target.value):null})}><option value="">No fixed off</option>{WEEKDAYS.map(x=><option key={x.value} value={x.value} disabled={x.value===6}>{x.label}{x.value===6?' · unavailable':''}</option>)}</select></label>
          </div>
