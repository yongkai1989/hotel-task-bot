...
type StabilitySnapshot = {
  ...
  scheduler: { runs_24h: number; failures_24h: number; latest_failure: string | null; retained_runs: number };
  cleanup_eligible: { completed_task_media: number; chiller_submissions: number };
};
type Payload = {
  usage: Snapshot;
  ...
  return (
    <main className="usage-page">
      ...
      {payload ? <>
        ...
        <section className="stability-grid">
          <article className={payload.stability.scheduler.failures_24h > 0 ? 'attention' : ''}>
            <span>Scheduler health</span>
            <strong>{payload.stability.scheduler.failures_24h.toLocaleString()} failed</strong>
            <small>{payload.stability.scheduler.runs_24h.toLocaleString()} runs during the past 24 hours</small>
          </article>
          <article>
            <span>Storage added · 30 days</span>
            <strong>{bytes(payload.stability.storage_growth_30d_bytes)}</strong>
            <small>Watch this together with the 1 GB storage meter</small>
          </article>
          <article>
            <span>Database connections</span>
            <strong>{payload.stability.connections.toLocaleString()}</strong>
            <small>Current project connections</small>
          </article>
          <article>
            <span>Next automatic cleanup</span>
            <strong>{(payload.stability.cleanup_eligible.completed_task_media + payload.stability.cleanup_eligible.chiller_submissions).toLocaleString()}</strong>
            <small>Eligible completed-task media and old cleaning submissions</small>
          </article>
        </section>
