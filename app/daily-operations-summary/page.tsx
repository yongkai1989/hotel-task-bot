...
type LinenItem = {
  ...
  in_bill: number;
  returned: number;
  bill_minus_return: number;
  bill_minus_maid: number;
...
export default function DailyOperationsSummaryPage() {
  ...
  const overdueRows = checklistRows.filter((row) => row.status === 'OVERDUE');
  ...
  const roomPercent = summary?.rooms?.linen_rooms_expected ? Math.round(summary.rooms.linen_rooms_saved * 100 / summary.rooms.linen_rooms_expected) : 100;
  ...
  return (
    <main className="ops-page">
      ...
      <section className="score-grid">
        ...
        <article className={roomPercent === 100 ? 'score good' : 'score warn'}><span>Pending linen rooms saved</span><strong>{roomPercent}%</strong><small>{summary?.rooms?.linen_rooms_saved || 0} of {summary?.rooms?.linen_rooms_expected || 0} rooms</small></article>
      ...
      <section className="panel attention-panel">
        <div className="panel-title"><div><span className="eyebrow red">ACT NOW</span><h2>Needs attention</h2></div><span>{attentionCount} exception(s)</span></div>
        {!attentionCount ? <div className="all-clear">All expected reports and operational saves are complete for this date.</div> : null}
        <div className="attention-list">
          {overdueRows.map((row) => <Link className="attention-item overdue" href={sourceMeta(row.source_type).href} key={`${row.source_type}-${row.template_id}`}><div><strong>{row.owner_name}</strong><span>{sourceMeta(row.source_type).label} - {row.title}</span></div><em>OVERDUE {row.due_time ? `- ${formatTime(row.due_time)}` : ''}</em></Link>)}
          {missingRows.filter((row) => row.status === 'MISSING').map((row) => <Link className="attention-item" href={sourceMeta(row.source_type).href} key={`${row.source_type}-${row.template_id}`}><div><strong>{row.owner_name}</strong><span>{sourceMeta(row.source_type).label} - {row.title}</span></div><em>NOT SUBMITTED</em></Link>)}
          {missingRooms.length ? <Link className="attention-item" href="/dashboard/laundry-count"><div><strong>{missingRooms.length} pending room(s) not saved</strong><span>Rooms {missingRooms.join(', ')}</span></div><em>LINEN ENTRY</em></Link> : null}
          {summary && !summary.linen.bill_saved ? <Link className="attention-item" href="/dashboard/laundry-count"><div><strong>In Bill is incomplete</strong><span>{summary.linen.bill_saved_rows} of {summary.linen.bill_expected_rows} floor records saved</span></div><em>NOT SAVED</em></Link> : null}
          {summary && !summary.linen.return_saved ? <Link className="attention-item" href="/dashboard/laundry-count"><div><strong>Laundry Return is incomplete</strong><span>{summary.linen.return_saved_rows} of {summary.linen.return_expected_rows} block records for {formatDate(summary.linen.return_service_date)}</span></div><em>NOT SAVED</em></Link> : null}
          {summary?.rooms?.open_manager_room_checks ? <Link className="attention-item" href="/dashboard/hk-manager-room-check"><div><strong>{summary.rooms.open_manager_room_checks} open manager room check(s)</strong><span>{summary.rooms.open_manager_rooms.length ? `Rooms ${summary.rooms.open_manager_rooms.join(', ')}` : 'Follow-up is still open'}</span></div><em>ROOM CHECK</em></Link> : null}
          {projectExceptions.map((project) => <Link className={`attention-item ${project.status === 'OVERDUE' ? 'overdue' : ''}`} href="/dashboard/hk-special-project" key={project.run_id}><div><strong>{project.title}</strong><span>{project.done_rooms}/{project.total_rooms} rooms - due {formatDate(project.due_date)}</span></div><em>{project.status === 'OVERDUE' ? 'PROJECT OVERDUE' : 'NO MOVEMENT'}</em></Link>)}
        </div>
      </section>
      ...
      <section className="panel">
        <div className="panel-title"><div><span className="eyebrow">LINEN RECONCILIATION</span><h2>What the differences mean</h2><p>Positive means In Bill is higher. Negative means usage or return is higher than In Bill.</p></div><Link href="/dashboard/laundry-count">Open Linen Count</Link></div>
        <div className="table-wrap"><table className="linen-table"><thead><tr><th>Linen</th><th>Maid use</th><th>PA use</th><th>Total use</th><th>In Bill</th><th>Return</th><th>Bill - Return</th><th>Bill - Maid</th><th>Bill - Total use</th></tr></thead><tbody>{(summary?.linen?.items || []).map((item) => <tr key={item.key}><td><b>{item.label}</b></td><td>{item.maid_use}</td><td>{item.pa_use}</td><td>{item.total_use}</td><td>{item.in_bill}</td><td>{item.returned}</td><td><Diff value={item.bill_minus_return} /></td><td><Diff value={item.bill_minus_maid} /></td><td><Diff value={item.bill_minus_total_use} /></td></tr>)}</tbody></table></div>
      </section>
      ...
      {ruleDraft ? <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setRuleDraft(null)}><section className="modal" role="dialog" aria-modal="true"><span className="eyebrow">SUPERUSER RULE</span><h2>{ruleDraft.row.title}</h2><p>Set who is expected, the deadline, and which days this submission is required.</p><label>Responsible name<input value={ruleDraft.ownerName} onChange={(event) => setRuleDraft({ ...ruleDraft, ownerName: event.target.value })} /></label><label>Responsible email<input type="email" value={ruleDraft.ownerEmail} onChange={(event) => setRuleDraft({ ...ruleDraft, ownerEmail: event.target.value })} /></label><label>Deadline<input type="time" value={ruleDraft.dueTime} onChange={(event) => setRuleDraft({ ...ruleDraft, dueTime: event.target.value })} /></label><div className="day-grid">{DAY_OPTIONS.map((day) => <label key={day.value}><input type="checkbox" checked={ruleDraft.activeDays.includes(day.value)} onChange={(event) => setRuleDraft({ ...ruleDraft, activeDays: event.target.checked ? [...ruleDraft.activeDays, day.value].sort() : ruleDraft.activeDays.filter((value) => value !== day.value) })} />{day.label}</label>)}</div><div className="modal-actions"><button className="secondary" onClick={() => setRuleDraft(null)}>Cancel</button><button onClick={() => void saveRule()} disabled={savingRule}>{savingRule ? 'Saving...' : 'Save Rule'}</button></div></section></div> : null}
      <Styles />
    </main>
  );
}
