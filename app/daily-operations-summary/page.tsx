...
export default function DailyOperationsSummaryPage() {
  ...
  return (
    <main className="ops-page">
      ...
      {ruleDraft ? <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setRuleDraft(null)}><section className="modal" role="dialog" aria-modal="true"><span className="eyebrow">SUPERUSER RULE</span><h2>{ruleDraft.row.title}</h2><p>{ruleDraft.row.source_type === 'SUPERVISOR_CHECKLIST' ? 'All three Housekeeping Supervisor accounts are expected. Their current names come from Admin Settings; set the shared deadline and required days here.' : 'Set who is expected, the deadline, and which days this submission is required.'}</p>{ruleDraft.row.source_type !== 'SUPERVISOR_CHECKLIST' ? <><label>Responsible name<input value={ruleDraft.ownerName} onChange={(event) => setRuleDraft({ ...ruleDraft, ownerName: event.target.value })} /></label><label>Responsible email<input type="email" value={ruleDraft.ownerEmail} onChange={(event) => setRuleDraft({ ...ruleDraft, ownerEmail: event.target.value })} /></label></> : null}<label>Deadline<input type="time" value={ruleDraft.dueTime} onChange={(event) => setRuleDraft({ ...ruleDraft, dueTime: event.target.value })} /></label><div className="day-grid">{DAY_OPTIONS.map((day) => <label key={day.value}><input type="checkbox" checked={ruleDraft.activeDays.includes(day.value)} onChange={(event) => setRuleDraft({ ...ruleDraft, activeDays: event.target.checked ? [...ruleDraft.activeDays, day.value].sort() : ruleDraft.activeDays.filter((value) => value !== day.value) })} />{day.label}</label>)}</div><div className="modal-actions"><button className="secondary" onClick={() => setRuleDraft(null)}>Cancel</button><button onClick={() => void saveRule()} disabled={savingRule}>{savingRule ? 'Saving...' : 'Save Rule'}</button></div></section></div> : null}
      <Styles />
    </main>
  );
}
